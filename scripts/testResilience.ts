...
async function testBackupRetention() {
  ...
  try {
    ...
    if (Math.abs(actualExpiration - expectedExpiration) > 1000) {
      throw new Error(
        `Backup expiration mismatch: expected ${expectedExpiration}, got ${actualExpiration}`
      )
    }
...
async function testAIResponseValidation() {
  ...
  try {
    ...
    const testCases = [
      ...
      {
        content: 'A longer response that should be valid because it has enough content',
        expected: true,
        name: 'long valid content',
      },
    ]

    for (const testCase of testCases) {
      const isValid = (
        service as unknown as { validateResponse: (content: string) => boolean }
      ).validateResponse(testCase.content)
      if (isValid !== testCase.expected) {
        throw new Error(
          `Validation failed for ${testCase.name}: expected ${testCase.expected}, got ${isValid}`
        )
      }
    }
...
async function testTransientErrorDetection() {
  ...
  try {
    ...
    const nonTransientErrors = ['Invalid API key', 'Authentication failed', 'Permission denied']
