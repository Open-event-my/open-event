...
const ipAddress = (): fc.Arbitrary<string> => {
  return fc.oneof(
    ...
    fc
      ...
      .map(([a, b, c, d]) => `${a}.${b}.${c}.${d}`),
    // IPv6 (simplified)
    fc.string({ minLength: 4, maxLength: 4 }).map(
      (s) =>
        `2001:db8::${s
          .split('')
          .map((_, i) => '0123456789abcdef'[i % 16])
    ...
    fc.record({
      ...
      metadata: fc.option(
        fc.record({
          ...
          provider: fc.option(fc.constantFrom('google', 'password'), { nil: undefined }),
          reason: fc.option(
            fc.constantFrom('invalid_credentials', 'account_locked', 'rate_limited'),
            { nil: undefined }
          ),
        }),
        { nil: undefined }
    ...
    fc.record({
      ...
      metadata: fc.option(
        fc.record({
          authType: fc.option(fc.constantFrom('oauth', 'credentials', 'email'), { nil: undefined }),
          provider: fc.option(fc.constantFrom('google', 'password'), { nil: undefined }),
          reason: fc.option(fc.constantFrom('email_exists', 'invalid_email', 'weak_password'), {
            nil: undefined,
          }),
        }),
        { nil: undefined }
...
describe('Authentication Event Logging Property Tests', () => {
  ...
  describe('Property 10: Authentication Event Logging', () => {
    it('should create log entry with required fields for any auth event', () => {
      fc.assert(
        fc.property(authLogEntry(), (logEntry) => {
    ...
    it('should handle concurrent auth events without data loss', () => {
      fc.assert(
        fc.property(fc.array(authLogEntry(), { minLength: 1, maxLength: 10 }), (logEntries) => {
