...
export class AIResilienceService {
  ...
  async callWithFallback(
    ...
    try {
      ...
      return {
        content: 'I apologize, but I\'m currently unable to process your request due to technical difficulties. Please try again in a few moments.',
        provider: 'fallback',
        cached: false,
      };
    }
      ...
      const result = await circuitBreaker.execute(async () => {
        ...
        return await this.retryWithBackoff(async () => {
          return await this.executeWithTimeout(async () => {
            ...
            // Call AI provider
            const stream = await aiProvider.createStreamingChat(
              messages,
              tools,
              {
                model: config.model,
