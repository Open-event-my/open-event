...
import type { AIMessage, ProviderCredentials, ProviderType } from '../ai/types'
...
describe('AI Resilience Service - Property Tests', () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (messages, provider, model) => {
          ...
          vi.spyOn(
            service as unknown as { callProvider: () => Promise<unknown> },
            'callProvider'
          ).mockRejectedValue(new Error('Provider unavailable'))

          const response = await service.callWithFallback(messages as AIMessage[], [], config)
  ...
  it('should validate all AI responses before returning', async () => {
    await fc.assert(
      fc.asyncProperty(fc.string(), (responseContent) => {
        const isValid = (
          service as unknown as { validateResponse: (r: string) => boolean }
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (maxRetries, baseDelay) => {
          ...
          const originalSleep = (testService as unknown as { sleep: (ms: number) => Promise<void> })
            .sleep
          vi.spyOn(
            testService as unknown as { sleep: (ms: number) => Promise<void> },
            'sleep'
      ...
      fc.asyncProperty(
        ...
        async (timeout, executionTime) => {
          ...
          try {
            ...
            ).executeWithTimeout(slowFn, timeout)
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (prompt, provider, model, responseContent) => {
          ...
          const notCachedDifferentProvider = service.getCachedResponse(prompt, differentConfig)
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (prompt, provider, model) => {
          ...
          const key1 = (
            service as unknown as { getCacheKey: (prompt: string, config: AIConfig) => string }
          ).getCacheKey(prompt, config)
          const key2 = (
            service as unknown as { getCacheKey: (prompt: string, config: AIConfig) => string }
          ).getCacheKey(prompt, config)
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        (errorMessage) => {
          ...
          const isTransient = (
            service as unknown as { isTransientError: (error: Error) => boolean }
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (messages, provider, model) => {
          ...
          await service.callWithFallback(messages as AIMessage[], [], config)
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (provider, model) => {
          ...
          vi.spyOn(
            testService as unknown as { callProvider: () => Promise<unknown> },
            'callProvider'
          ).mockResolvedValue({
