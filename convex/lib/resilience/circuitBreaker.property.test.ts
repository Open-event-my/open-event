...
import { CircuitBreaker, CircuitBreakerConfig, createAICircuitBreaker } from './circuitBreaker';
...
describe('Property 31: AI Circuit Breaker', () => {
  ...
  test('circuit should close after success threshold in half-open state', { timeout: 10000 }, async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.integer({ min: 2, max: 5 }), // failure threshold
        fc.integer({ min: 1, max: 5 }), // success threshold
