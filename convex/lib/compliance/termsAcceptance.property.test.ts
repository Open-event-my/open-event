...
const termsAcceptanceArbitrary = (userId: string) =>
  fc.record({
    _id: fc.string({ minLength: 10, maxLength: 20 }),
    userId: fc.constant(userId),
    version: versionArbitrary,
    acceptedAt: timestampArbitrary,
          ...
          async (user, version, attemptCount) => {
            ...
            for (let i = 0; i < attemptCount; i++) {
              ...
              const existing = Array.from(ctx.mockData.termsAcceptance.values()).find(
                (a: any) => a.userId === user._id && a.version === version
              );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (user, versions) => {
            ...
            const acceptedVersions = acceptances.map((a: any) => a.version);
        ...
        fc.asyncProperty(
          ...
          async (user, acceptanceData) => {
            ...
            acceptances.forEach((acceptance: any) => {
              expect(acceptance.userId).toBe(user._id);
              expect(acceptance.version).toBeDefined();
              expect(acceptance.acceptedAt).toBeGreaterThan(0);
            });
          }
        ),
