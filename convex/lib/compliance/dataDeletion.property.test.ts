...
interface MockUser {
  ...
  email: string
  name?: string
  role: 'organizer' | 'admin' | 'superadmin'
  status: 'active' | 'suspended' | 'pending'
  createdAt: number
}

interface MockOrganizerProfile {
  _id: string
  userId: string
...
interface MockData {
  ...
  organizerProfiles: Map<string, MockOrganizerProfile>
  ...
  organizationMembers: Map<string, MockOrganizationMember>
  orders: Map<string, MockRecord>
  attendees: Map<string, MockRecord>
  apiKeys: Map<string, MockApiKey>
  notes: Map<string, MockRecord>
  notifications: Map<string, MockNotification>
  budgetItems: Map<string, MockBudgetItem>
  eventTasks: Map<string, MockEventTask>
  inquiries: Map<string, MockRecord>
  eventApplications: Map<string, MockRecord>
  auditLogs: Map<string, MockRecord>
  sessions: Map<string, MockSession>
  verificationTokens: Map<string, MockRecord>
  eventSponsors: Map<string, MockRecord>
  eventVendors: Map<string, MockRecord>
  ticketTypes: Map<string, MockRecord>
}
...
describe('Data Deletion Service - Property Tests', () => {
  describe('Property 14: Complete Data Deletion', () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (user, testData) => {
            ...
            const profilesToDelete = Array.from(ctx.mockData.organizerProfiles.values()).filter(
              (p) => p.userId === user._id
            )
            profilesToDelete.forEach((p) => ctx.mockData.organizerProfiles.delete(p._id))

            const prefsToDelete = Array.from(ctx.mockData.notificationPreferences.values()).filter(
              (p) => p.userId === user._id
            )
            prefsToDelete.forEach((p) => ctx.mockData.notificationPreferences.delete(p._id))

            const eventsToDelete = Array.from(ctx.mockData.events.values()).filter(
              (e) => e.organizerId === user._id
            )
            eventsToDelete.forEach((e) => {
              // Delete event-related data
              const budgets = Array.from(ctx.mockData.budgetItems.values()).filter(
                (b) => b.eventId === e._id
              )
              budgets.forEach((b) => ctx.mockData.budgetItems.delete(b._id))

              const tasks = Array.from(ctx.mockData.eventTasks.values()).filter(
                (t) => t.eventId === e._id
              )
              tasks.forEach((t) => ctx.mockData.eventTasks.delete(t._id))

              ctx.mockData.events.delete(e._id)
            })

            const notifsToDelete = Array.from(ctx.mockData.notifications.values()).filter(
              (n) => n.userId === user._id
            )
            notifsToDelete.forEach((n) => ctx.mockData.notifications.delete(n._id))

            const sessionsToDelete = Array.from(ctx.mockData.sessions.values()).filter(
              (s) => s.userId === user._id
            )
            sessionsToDelete.forEach((s) => ctx.mockData.sessions.delete(s._id))

            const keysToDelete = Array.from(ctx.mockData.apiKeys.values()).filter(
              (k) => k.userId === user._id
            )
            keysToDelete.forEach((k) => ctx.mockData.apiKeys.delete(k._id))
            ...
            const remainingProfiles = Array.from(ctx.mockData.organizerProfiles.values()).filter(
              (p) => p.userId === user._id
            )
            expect(remainingProfiles).toHaveLength(0)

            const remainingPrefs = Array.from(ctx.mockData.notificationPreferences.values()).filter(
              (p) => p.userId === user._id
            )
            expect(remainingPrefs).toHaveLength(0)

            const remainingEvents = Array.from(ctx.mockData.events.values()).filter(
              (e) => e.organizerId === user._id
            )
            expect(remainingEvents).toHaveLength(0)

            const remainingNotifs = Array.from(ctx.mockData.notifications.values()).filter(
              (n) => n.userId === user._id
            )
            expect(remainingNotifs).toHaveLength(0)

            const remainingSessions = Array.from(ctx.mockData.sessions.values()).filter(
              (s) => s.userId === user._id
            )
            expect(remainingSessions).toHaveLength(0)

            const remainingKeys = Array.from(ctx.mockData.apiKeys.values()).filter(
              (k) => k.userId === user._id
            )
            ...
            for (const eventId of eventIds) {
              const remainingBudgets = Array.from(ctx.mockData.budgetItems.values()).filter(
                (b) => b.eventId === eventId
              )
              expect(remainingBudgets).toHaveLength(0)

              const remainingTasks = Array.from(ctx.mockData.eventTasks.values()).filter(
                (t) => t.eventId === eventId
              )
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (user, testData) => {
            ...
            for (const eventId of eventIds) {
              const orphanedBudgets = Array.from(ctx.mockData.budgetItems.values()).filter(
                (b) => b.eventId === eventId
              )
              expect(orphanedBudgets).toHaveLength(0)

              const orphanedTasks = Array.from(ctx.mockData.eventTasks.values()).filter(
                (t) => t.eventId === eventId
              )
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (user, testData) => {
            ...
            if (org) {
              const otherAdmins = Array.from(ctx.mockData.organizationMembers.values()).filter(
                (m) =>
              ...
              if (otherAdmins.length > 0) {
                ...
                const members = Array.from(ctx.mockData.organizationMembers.values()).filter(
                  (m) => m.organizationId === orgId
                )
                members.forEach((m) => ctx.mockData.organizationMembers.delete(m._id))
                ctx.mockData.organizations.delete(orgId)
              }
            }

            // Delete user's membership
            const userMemberships = Array.from(ctx.mockData.organizationMembers.values()).filter(
              (m) => m.userId === user._id
            )
            userMemberships.forEach((m) => ctx.mockData.organizationMembers.delete(m._id))
            ...
            if (testData.hasOtherAdmin) {
              ...
              const remainingMembers = Array.from(ctx.mockData.organizationMembers.values()).filter(
                (m) => m.organizationId === orgId
              )
    ...
    it('should handle deletion of non-existent users gracefully', async () => {
      await fc.assert(
        fc.asyncProperty(userIdArbitrary, async (nonExistentUserId) => {
          const ctx = createMockContext()
