      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (user, eventIds) => {
            ...
            const exportedEvents = await ctx.db.query('events').withIndex('by_organizer', (q: any) => q.eq('organizerId', user._id)).collect();
            
            // Verify all events are exported
            expect(exportedEvents).toHaveLength(events.length);
            
            // Verify each event belongs to the user
            exportedEvents.forEach((event: any) => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (user, orgIds) => {
            ...
            const exportedOrgs = await ctx.db.query('organizations').withIndex('by_owner', (q: any) => q.eq('ownerId', user._id)).collect();
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (user, orderTemplates) => {
            ...
            const exportedOrders = await ctx.db.query('orders').withIndex('by_email', (q: any) => q.eq('buyerEmail', user.email)).collect();
