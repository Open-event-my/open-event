...
const largeTransactionRule: FraudRule = {
  ...
  check: (input) => {
    ...
    return {
      ...
      reason: triggered 
        ? `Large transaction amount: $${(input.amount / 100).toFixed(2)}`
        : '',
...
const velocityRule: FraudRule = {
  ...
  riskScore: 40,
  check: (input) => {
    const triggered = input.timeSinceLastOrder !== undefined && 
                     input.timeSinceLastOrder < FRAUD_THRESHOLDS.velocityTimeThreshold;
    const isVeryRapid = input.timeSinceLastOrder !== undefined && 
                       input.timeSinceLastOrder < 30000; // Under 30 seconds
...
const unusualSpendingRule: FraudRule = {
  ...
  riskScore: 15,
  check: (input) => {
    if (input.previousTotalSpent === undefined || 
...
const failedAttemptsRule: FraudRule = {
  ...
  riskScore: 35,
  check: (input) => {
    const triggered = input.failedAttemptsLastHour !== undefined && 
...
const highRiskCountryRule: FraudRule = {
  ...
  riskScore: 15,
  check: (input) => {
    const triggered = input.countryCode !== undefined && 
