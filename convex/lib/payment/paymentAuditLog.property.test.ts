...
describe('Payment Audit Logging Property Tests', () => {
  ...
  describe('Property 47: Payment Event Audit Logging', () => {
    ...
    const orderNumberArb = fc
      ...
      .map((s) => `ORD-${s.replace(/[^a-zA-Z0-9]/g, 'x').toUpperCase()}`)
    ...
    const stripeIdArb = (prefix: string) =>
      fc
        .string({ minLength: 10, maxLength: 24 })
        .map((s) => `${prefix}${s.replace(/[^a-zA-Z0-9]/g, 'x')}`)
    ...
    const paymentAuditEntryArb: fc.Arbitrary<PaymentAuditEntry> = fc.record({
      ...
      paymentMethod: fc.option(fc.constantFrom('card', 'bank_transfer', 'wallet'), {
        nil: undefined,
      }),
      cardBrand: fc.option(fc.constantFrom('visa', 'mastercard', 'amex', 'discover'), {
        nil: undefined,
      }),
      cardLast4: fc.option(
        fc.string({ minLength: 4, maxLength: 4 }).map((s) => s.replace(/\D/g, '0').slice(0, 4)),
        { nil: undefined }
      ),
      refundReason: fc.option(fc.string({ minLength: 5, maxLength: 100 }), { nil: undefined }),
      disputeReason: fc.option(
        fc.constantFrom('fraudulent', 'duplicate', 'product_not_received', 'general'),
        { nil: undefined }
      ),
      ...
      fc.assert(
        fc.property(
          ...
          (orderId, orderNumber, amount, currency) => {
            ...
            const validation = validatePaymentAuditEntry(entry)
            return (
              validation.isValid === false && validation.errors.some((e) => e.includes('eventType'))
            )
          }
        ),
        { numRuns: 100 }
      ...
      fc.assert(
        fc.property(
          ...
          (eventType, orderNumber, amount, currency) => {
            ...
            const validation = validatePaymentAuditEntry(entry)
            return (
              validation.isValid === false && validation.errors.some((e) => e.includes('orderId'))
            )
          }
        ),
        { numRuns: 100 }
      ...
      fc.assert(
        fc.property(
          ...
          (eventType, orderId, orderNumber, amount, currency) => {
            ...
            const validation = validatePaymentAuditEntry(entry)
            return (
              validation.isValid === false && validation.errors.some((e) => e.includes('amount'))
            )
          }
        ),
        { numRuns: 100 }
      ...
      fc.assert(
        fc.property(
          ...
          (eventType, orderId, orderNumber, amount) => {
            ...
            const validation = validatePaymentAuditEntry(entry)
            return (
              validation.isValid === false && validation.errors.some((e) => e.includes('currency'))
            )
          }
        ),
        { numRuns: 100 }
    ...
    it('should handle null/undefined metadata gracefully', () => {
      fc.assert(
        fc.property(fc.constantFrom(null, undefined), (metadata) => {
          ...
          fc.record({
            level1: fc.record({
              level2: fc.record({
                ...
                cardNumber: fc.option(fc.string({ minLength: 16, maxLength: 16 }), {
                  nil: undefined,
                }),
              }),
            }),
