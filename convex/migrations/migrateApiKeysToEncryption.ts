...
export const revokeUnmigratedApiKeys = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const unmigratedKeys = apiKeys.filter(
      (key) => key.description?.includes('MIGRATION NOTICE')
    );
...
export const getMigrationStatus = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const unmigratedKeys = activeKeys.filter(
      (k) => k.description?.includes('MIGRATION NOTICE')
    );
    const migratedKeys = activeKeys.filter(
      (k) => !k.description?.includes('MIGRATION NOTICE')
    );
    ...
    return {
      ...
      migrationProgress: activeKeys.length > 0 
        ? Math.round((migratedKeys.length / activeKeys.length) * 100)
        : 100,
    };
  },
});
